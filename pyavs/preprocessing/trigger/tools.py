...
# Module logger
logger = get_logger('preprocessing.trigger.tools')
...
def repair_meg_trigger_events(events: np.ndarray, session: int,
    ...
    """
    # Policy: we only relabel corrupt block triggers that are physically present in the
    # raw signal.  We never synthesise or interpolate missing triggers.  If a trigger
    # event is absent because the STI channel did not transition (e.g. the block trigger
    # code equals the trial-number code, or the block trigger maps to 0), the affected
    # trial is simply absent from the data and is excluded from all downstream analyses.
    # Attempting to reconstruct missing triggers would introduce uncertain timing and
    # risk contaminating neighbouring events.
    #
    # Known irrecoverable cases:
    #   - Session 4 block 50 : block trigger code == scene_on (100), invisible in signal.
    #   - Session 6 block 78 : block trigger code == 0, undetectable by find_events.
    #   - Sessions 6–8 blocks 79–108: block trigger code == trial-number code (1–30) for
    #     one specific trial per block; that trial-number event is absent from the signal.
    ...
    for block_trigger_idx, block_trigger in enumerate(block_triggers_this_session):
        ...
        if block_trigger <= 30:
            # The block trigger code collides with a trial-number code (1–30).
            # We identify corrupt instances by prev_code == scene_on (100), and
            # relabel only those.  The trial-number event that would share the same
            # code is physically absent from the raw signal: when the STI channel
            # is already at `block_trigger` after the corrupt block trigger fires,
            # sending the same value again produces no channel transition, so
            # find_events never records it.  Per pipeline policy we do NOT replace
            # that missing trial-number event; the affected trial is simply skipped.
            scene_onset_indices = np.where(events_with_block_trigger[:, 1] == meg_trigger_dict['scene_on'])[0]

            # All events with this code preceded by scene_on are corrupt block
