...
def main():
    ...
    logger = get_logger('examples.meg_et_workflow_simple')
    
    logger.info("=== pyAVS MEG + Eye Tracking Workflow (Streamlined) ===")
    ...
    try:
        ...
        logger.info(f"   Loaded MEG blocks: {list(subject_data['meg_data'].keys())}")
        logger.info(f"   Loaded {len(subject_data['eye_events'])} eye tracking events")
    ...
    try:
        ...
        logger.info(f"   Created {len(saccade_epochs)} saccade epochs")
        ...
        logger.info(f"   Created {len(fixation_epochs)} fixation epochs")
    ...
    try:
        ...
        logger.info(f"   Computed source estimates: {len(source_estimates)} epochs")
        ...
        logger.info(f"   Extracted {len(roi_data)} ROI datasets")
        
    except FileNotFoundError:
        logger.warning("   Forward model not found - skipping source reconstruction")
        source_estimates = None
    except Exception as e:
        logger.error(f"   Error in source reconstruction: {e}")
        source_estimates = None
    ...
    try:
        if source_estimates is not None:
            ...
            logger.info(f"   Population codes saved: {os.path.basename(h5_path)}")
        
        # Save epochs in .fif format
        epochs_path = pyavs.save_source_data(
            saccade_epochs, data_type='saccade_epochs'
        )
        logger.info(f"   Epochs saved: {os.path.basename(epochs_path)}")
    ...
    try:
        ...
        logger.info(f"   Found {len(found_files)} population codes files for subject {subject_id}")
        
        # List available parameter sets
        param_sets = pyavs.list_available_parameter_sets()
        logger.info(f"   Found {len(param_sets)} unique parameter sets in storage")
        
    except Exception as e:
        logger.error(f"   Error in file discovery: {e}")
    
    logger.info("\n=== Workflow Complete ===")
    logger.info("This streamlined example demonstrated:")
    logger.info("- Data loading and preprocessing with causal filtering option")
    logger.info("- MEG-ET epoch creation with robust alignment")
    logger.info("- Source reconstruction with beamformer method")
    logger.info("- Intelligent storage with parameter tracking")
    logger.info("- File discovery and parameter set management")


def minimal_example():
    """Minimal working example - just the essentials."""
    
    logger = get_logger('examples.meg_et_workflow_simple.minimal')
    ...
    logger.info(f"Processed {len(epochs)} saccade epochs")
